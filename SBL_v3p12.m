...
% Minimizing (YY^T / AGA^T + penality) should lead to the correct replica selection (up to a bogus scale factor/amplitude).
...
%
% Version 3.1
% Different convergance norm and code update. A and Y have now one more dimensions Posterior unbiased mean
% Handles single snapshot. Version 3.12
% more efficient diagonal gamma computations
% Kay L Gemba & Santosh Nannuru MPL/SIO/UCSD gemba@ucsd.edu & snannuru@ucsd.edu
%% check function
% [1] SEQUENTIAL SPARSE BAYESIAN LEARNING FOR DOA
% [2] Y. Park, F. Meyer, and P. Gerstoft, “Sequential sparse Bayesian learning for time-varying direction of arrival,” J. Acoust. Soc. Am. 149(3) (2021).
...
for j1 = 1 : options.convergence.maxiter    
    ...
    for i_f = 1 : Nfreq        
        ...
        % c.f.  7th and 9th line below (5) of [1], numerator  
%         ApSigmaYinv1  = Af' *inv(sigc(i_f) * eye(Nsensor) + Af * (repmat(gamma, [1 Nsensor] ) .* Af'));     
%         sum(sum(abs(ApSigmaYinv1-ApSigmaYinv))); 
        % Sum over snapshots and normalize, abs for roundoff errors
        gamma_num(i_f,:)   = sum ( abs ( ( ApSigmaYinv * squeeze2(Y(i_f,:,:)) ).^2 ),2 ) / Nsnapshot;        
        % positive def quantity, abs for roundoff errors,                   % c.f.  7th line below (5) of [1], numerator 
        gamma_denum(i_f,:) = abs( sum  ( ApSigmaYinv.' .* Af, 1 ) );%c.f.  7th line below (5) of [1], denominator 
    end    
    % Fixed point Eq. update
    gamma = gamma  .* ((sum( gamma_num   ,1 ) ./sum( gamma_denum ,1 ) ).^(1/options.fixedpoint) ).' ;    %c.f.  7 line below (5) of [1], denominator 
...
%      % 15 lines below (5) of [1]
    end    
    %% Convergance
    % checks convergance and displays status reports,  convergance indicator
...
%         tic 
%         DimNum=8000;
%         dd =rand(DimNum,DimNum)/rand(DimNum,DimNum);
%         toc
%           tic         
%         dd =rand(DimNum,DimNum)*inv(rand(DimNum,DimNum));
%         toc        
