...
dbstop if error;
% addpath([cd,'/_common'])
% SNRlist = [40 35 30 25 20 15 10 5 0]; SNRlist = [40 35 30 25 20 15 10 5];
SNRlist = [20];
...
receivedSignal = zeros(Nsensor,Nsnapshot);source_amp = zeros(Nsources,Nsnapshot);
...
hold on; scatter(rtIndex,rtTheta,50,rtAmp,'filled','o','linewidth',.5,  'MarkerEdgeColor','k'); hold off;
...
%% Conventional beamforming (CBF)
figure;set(gcf,'position',[1,250,560,420]);imagesc(1:Nsnapshot,theta,10*log10( (abs(outputBeamformer).^2) ./ max((abs(outputBeamformer).^2),[],1)));
caxis([-20 0])
